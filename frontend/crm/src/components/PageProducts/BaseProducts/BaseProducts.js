...
const getCategoryUrl = (categoryId, parentId, page = "1") => {
    return `/products/base?categoryId=${categoryId}&parentId=${parentId}&page=${page}`;
};
const getSearchUrl = (categoryId, search, page = "1") => {
    return `/products/base?categoryId=${categoryId}&search=${search}&page=${page}`;
};
...
const BaseProducts = (props) => {
    ...
    useEffect(() => {
        ...
        if (parentCategories.length === 0 && querySearch === '') {
            breadcrumb.push({link: '/products/base', label: 'Базовые', active: true});
            onSetPath(breadcrumb);
            return;
        }
        if (querySearch !== '') {
            breadcrumb.push({link: '/products/base', label: 'Базовые', active: false});
...
export default connect(
    ...
    dispatch => ({
        ...
        onShowNewCategoryModal(categoryId = 0) {
            dispatch(newCategory(appApi.category.TYPE_BASE, categoryId, ()=>dispatch(findCategories())))
        },
